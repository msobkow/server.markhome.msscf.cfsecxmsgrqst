...
/*
 *	CFSecXMsgRqstISOCtryDelete XML SAX Element  implementation
 *	for ISOCtry.
 */
public class CFSecXMsgRqstISOCtryDelete
	extends CFLibXmlCoreElementHandler
{
	public CFSecXMsgRqstISOCtryDelete( CFSecXMsgRqst xmsgRqst ) {
		super( xmsgRqst );
	}
	...
	throws SAXException
	{
		...
		try {
			...
			// Delete the object
			ICFSecISOCtryObj read = ((ICFSecISOCtryObj)schemaObj.getISOCtryTableObj().readISOCtry( pkey ));
			if( read != null ) {
				if( read.getISOCtryBuff().getRequiredRevision() > natRevision ) {
					...
					ICFSecISOCtryEditObj editBuff = (ICFSecISOCtryEditObj)read.beginEdit();
					if( editBuff != null ) {
						editBuff.deleteInstance();
						editBuff = null;
						String response = schemaFormatter.formatRspnXmlPreamble() + "\n"
							+	"\t" + CFSecXMsgISOCtryMessageFormatter.formatISOCtryRspnDeleted() +	"\n"
