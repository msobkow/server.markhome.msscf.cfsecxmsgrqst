...
/*
 *	CFSecXMsgRqstISOLangRead XML SAX Element  implementation
 *	for ISOLang.
 */
public class CFSecXMsgRqstISOLangRead
	extends CFLibXmlCoreElementHandler
{
	public CFSecXMsgRqstISOLangRead( CFSecXMsgRqst xmsgRqst ) {
		super( xmsgRqst );
	}
	...
	throws SAXException
	{
		...
		try {
			...
			CFSecISOLangPKey pkey = ((ICFSecSchema)schemaObj.getBackingStore()).getFactoryISOLang().newPKey();
			...
			ICFSecISOLangObj read = ((ICFSecISOLangObj)schemaObj.getISOLangTableObj().readISOLang( pkey, true ));
			if( read != null ) {
				String response = schemaFormatter.formatRspnXmlPreamble() + "\n"
					+	"\t" + CFSecXMsgISOLangMessageFormatter.formatISOLangRspnSingleOpenTag()
					+	CFSecXMsgISOLangMessageFormatter.formatISOLangRspnDerivedRec( "\n\t\t", read.getISOLangBuff() ) + "\n"
					+	"\t" + CFSecXMsgISOLangMessageFormatter.formatISOLangRspnSingleCloseTag()
