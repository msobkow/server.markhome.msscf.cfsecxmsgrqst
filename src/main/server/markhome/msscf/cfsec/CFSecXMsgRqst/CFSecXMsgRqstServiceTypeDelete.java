...
/*
 *	CFSecXMsgRqstServiceTypeDelete XML SAX Element  implementation
 *	for ServiceType.
 */
public class CFSecXMsgRqstServiceTypeDelete
	extends CFLibXmlCoreElementHandler
{
	public CFSecXMsgRqstServiceTypeDelete( CFSecXMsgRqst xmsgRqst ) {
		super( xmsgRqst );
	}
	...
	throws SAXException
	{
		...
		try {
			...
			CFSecServiceTypePKey pkey = ((ICFSecSchema)schemaObj.getBackingStore()).getFactoryServiceType().newPKey();
			...
			// Delete the object
			ICFSecServiceTypeObj read = ((ICFSecServiceTypeObj)schemaObj.getServiceTypeTableObj().readServiceType( pkey ));
			if( read != null ) {
				if( read.getServiceTypeBuff().getRequiredRevision() > natRevision ) {
					...
					ICFSecServiceTypeEditObj editBuff = (ICFSecServiceTypeEditObj)read.beginEdit();
					if( editBuff != null ) {
						editBuff.deleteInstance();
						editBuff = null;
						String response = schemaFormatter.formatRspnXmlPreamble() + "\n"
							+	"\t" + CFSecXMsgServiceTypeMessageFormatter.formatServiceTypeRspnDeleted() +	"\n"
