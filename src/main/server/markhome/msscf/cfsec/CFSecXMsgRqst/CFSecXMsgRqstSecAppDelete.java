...
/*
 *	CFSecXMsgRqstSecAppDelete XML SAX Element  implementation
 *	for SecApp.
 */
public class CFSecXMsgRqstSecAppDelete
	extends CFLibXmlCoreElementHandler
{
	public CFSecXMsgRqstSecAppDelete( CFSecXMsgRqst xmsgRqst ) {
		super( xmsgRqst );
	}
	...
	throws SAXException
	{
		...
		try {
			...
			// Delete the object
			ICFSecSecAppObj read = ((ICFSecSecAppObj)schemaObj.getSecAppTableObj().readSecApp( pkey ));
			if( read != null ) {
				if( read.getSecAppBuff().getRequiredRevision() > natRevision ) {
					...
					ICFSecSecAppEditObj editBuff = (ICFSecSecAppEditObj)read.beginEdit();
					if( editBuff != null ) {
						editBuff.deleteInstance();
						editBuff = null;
						String response = schemaFormatter.formatRspnXmlPreamble() + "\n"
							+	"\t" + CFSecXMsgSecAppMessageFormatter.formatSecAppRspnDeleted() +	"\n"
